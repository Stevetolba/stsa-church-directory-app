...
export default function MembersPage() {
  ...
  const { data, isLoading } = useMembers({ search, status, campus, page });
  ...
  return (
    <div>
      <div className="mb-7 flex flex-wrap items-start justify-between gap-6">
        <div>
          <h1 className="font-heading text-3xl font-semibold text-brand-navy">Members</h1>
          <p className="mt-1 text-[14.5px] text-[#5B7185]">
            {total} of {overallTotal} members
          </p>
        </div>
      ...
      {!isLoading && profiles.length === 0 ? (
        <EmptyState message={`No members match "${search}".`} />
      ) : (
        <div className="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-[18px]">
          {profiles.map((profile, index) => (
            <MemberCard key={profile.id} profile={profile} index={index} />
          ))}
        </div>
      )}
