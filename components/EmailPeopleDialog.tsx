...
export interface EmailPeopleFilters {
  search: string;
  status: MemberStatus[];
  campus: Campus[];
  gradeFrom?: number;
...
// ADR-0014: the People-page counterpart to EmailParentsDialog. Recipients
// are the profiles' own emails (no parent-contact resolution needed) —
// otherwise the same compose → review → send flow, kept as a parallel
// component rather than a shared abstraction to match this codebase's
// existing pattern of duplicating People/Children page infrastructure
// (page.tsx, usePeople/useChildren, /api/profiles//api/children) rather
// than unifying it.
export function EmailPeopleDialog({
  open,
  onOpenChange,
  user,
  fromAddress,
  ...
  useEffect(() => {
    ...
    fetch(`/api/profiles?${params.toString()}`)
      ...
      .then((result: ProfileSearchResult) => {
        if (cancelled) return;
  ...
  async function handleSend() {
    ...
    try {
      ...
      toast.success(`Email sent to ${data.recipientCount} ${data.recipientCount === 1 ? "person" : "people"}.`);
      onOpenChange(false);
    } catch (err) {
  ...
  // domain), so the sender's name is the display name and their email is
  // Reply-To, not the visible From address. Mirrors lib/email.ts exactly.
  const fromField = (
    <div>
  ...
  const recipientSummary = isLoadingRecipients
    ...
    : recipients === null
      ...
      : recipients.length === 0
        ? "No email addresses found for this filter"
        : `${recipients.length} unique email${recipients.length === 1 ? "" : "s"}`;

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogTitle>{step === "compose" ? "Email People" : "Review Email"}</DialogTitle>
        <DialogDescription>
          {step === "compose"
            ? `Sends to the ${matchCount} ${matchCount === 1 ? "person" : "people"} matching your current filters.`
