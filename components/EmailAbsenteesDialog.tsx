...
export interface EmailAbsenteesFilters {
  seriesId: string;
  lastN: number;
...
// ADR-0015 (Phase 5): the compose UI for emailing a series' absentees —
// near-identical to EmailParentsDialog (components/EmailParentsDialog.tsx),
// same compose -> preview -> send shape and the same principle that "To" is
// never editable here: it's a read-only list computed from the same
// filters the send itself recomputes server-side, so the review step shows
// exactly what will go out. The one real difference is who the recipient
// resolves to — a child absentee's parent/guardian, but an adult absentee
// (an "everyone" series like Liturgy can have adult absentees) gets emailed
// directly — handled server-side by resolveAbsenteeEmails.
export function EmailAbsenteesDialog({
  open,
  onOpenChange,
  user,
  fromAddress,
  ...
  const {
    ...
    defaultValues: { subject: `We missed you at ${seriesTitle}` },
  });
  ...
  useEffect(() => {
    ...
    setBodyHtml("");
    setAttachments([]);
    reset({ subject: `We missed you at ${seriesTitle}` });
    ...
    fetch(`/api/attendance/absentees?${params.toString()}`)
      .then((res) => (res.ok ? res.json() : Promise.reject(new Error(String(res.status)))))
      .then((result: { recipientEmails: string[] }) => {
        if (cancelled) return;
    ...
    // filters/seriesTitle are stable for the life of the dialog being open;
    // re-fetching recipients only needs to happen when it opens.
  ...
  async function handleSend() {
    ...
    try {
      ...
      toast.success(`Email sent to ${data.recipientCount} recipient${data.recipientCount === 1 ? "" : "s"}.`);
      onOpenChange(false);
    } catch (err) {
  ...
  const recipientSummary = isLoadingRecipients
    ...
    : recipients === null
      ...
      : recipients.length === 0
        ? "No email addresses found for this absentee list"
        : `${recipients.length} unique email${recipients.length === 1 ? "" : "es"}`;

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogTitle>{step === "compose" ? "Email Absentees" : "Review Email"}</DialogTitle>
        <DialogDescription>
          {step === "compose"
            ? `Sends to the parents/guardians (or the person themselves, if an adult) of the ${absenteeCount} ${absenteeCount === 1 ? "person" : "people"} who missed the last ${filters.lastN} occurrence${filters.lastN === 1 ? "" : "s"} of ${seriesTitle}.`
