...
**Kiosk device actor.** Admins generate a one-time setup code; entering it on an iPad/phone (`/kiosk/setup`) exchanges it for a long-lived device token, stored only as a sha256 hash, delivered in an httpOnly+secure `kiosk_device` cookie. A device actor can list today's events, search the roster with a **names-only projection** (server strips phone/email/DOB/allergy/care-notes — not just UI hiding), and check in/out for the selected event. Data that a device's roster search never carries but the flow still needs is resolved server-side at the point of use instead of being included in every search result: `GET /api/kiosk/roster` takes the target `eventId` as a query param and returns a `suggestedSessions` map (profile id → session id) computed from the profile's real DOB/grade server-side, so date of birth never has to leave the server for session auto-selection to work; `POST /api/kiosk/attendance` returns the checked-in child's allergy/care notes and the drop-off adult's phone number *only in the response to that specific check-in*, for the printed label — not as part of the searchable roster. It can never reach `/api/profiles`, `/api/households`, reports, or email. Devices are revocable and expose `last_seen_at`.
...
- A shared-iPad device token is a bearer credential. Mitigations: httpOnly+secure cookie, hash-only storage, admin revocation, `last_seen_at` visibility, a true names-only roster projection (no phone/email/DOB/allergy/care-notes in search results — session suggestion and label data are resolved server-side per-request instead, never bulk-exposed), kiosk-scoped endpoints. Residual risk: a stolen unlocked device can check people in/out and, per check-in it performs, see that one child's allergy/care notes and drop-off adult's phone on the printed label — but cannot browse the directory for that data, and gains nothing by searching alone.
